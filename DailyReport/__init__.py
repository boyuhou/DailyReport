...
def combine_two_image(image_list, target_image_path):
...
class GridBuilder:
        ...
        if is_columns_multiindex:
            ...
            for cur_section in column_level_sections:
                cell_section_config = cell_format_config[
                    ...
                    cur_section]
                col_dict[cur_section] = GridBuilder.__get_column_structure(GridBuilder.__get_df_col_segment(df, cur_section), cell_section_config,
            ...
            if len(cell_format_config) > 1:
                for column in df.columns:
                    cell_section_config = cell_format_config[CELL_FORMAT_DEFAULT_KEY] if column not in cell_format_config else cell_format_config[
                        column]
                    col_dict[column] = GridBuilder.__get_column_structure(df[[column]], cell_section_config, cur_row)
                col_dict[DICT_KEY_ALL] = GridBuilder.__merge_formatted_multiindex_df_into_one(df, col_dict, is_row=False)
            else:
                col_dict[DICT_KEY_ALL] = GridBuilder.__get_column_structure(df, cell_format_config[CELL_FORMAT_DEFAULT_KEY], cur_row)
    ...
    def __get_row_structure(df, cell_format_config, row_group=None):
        ...
        for cur_row_data in df.index:
            row_id = row_id + 1
            col_dict = GridBuilder.__get_column_structure_by_section(GridBuilder.__get_df_row_segment(df, cur_row_data), cell_format_config, row_id, row_group)
            col_dict['label'] = RichText(cur_row_data,
                                         bold=cell_format_config[CELL_FORMAT_DEFAULT_KEY].get('make_labels_bold', False),
                                         size=cell_format_config[CELL_FORMAT_DEFAULT_KEY].get('row_label_font_size', 8))
            # if background colour is specified:
            if (CELL_FORMAT_DEFAULT_KEY in cell_format_config) & ('background_colour_ftn' in cell_format_config[CELL_FORMAT_DEFAULT_KEY]):
                background_colour = cell_format_config[CELL_FORMAT_DEFAULT_KEY]['background_colour_ftn'](None, None, None, None, row_id)
    ...
    def __build_structure(df, cell_format_config):
        ...
        if is_row_multiindex:
            ...
            for cur_section in row_level_sections:
                data_dict[cur_section] = GridBuilder.__get_row_structure(GridBuilder.__get_df_row_segment(df, cur_section), cell_format_config,
                                                                         cur_section)
        ...
        if is_columns_multiindex:
            ...
            for cur_section in column_level_sections:
                column_dict[cur_section] = GridBuilder.__get_rich_text_columns(GridBuilder.__get_df_col_segment(df, cur_section).columns.tolist(),
                                                                               cell_format_config)
